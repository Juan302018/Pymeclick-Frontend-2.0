...
export class CalificacionEdicionComponent implements OnInit {
  ...
  operar() {
    ...
    if (this.calificacion != null && this.calificacion.id_calificacion > 0) {
      this.calificacionService.modificar(this.calificacion).pipe(
          switchMap(() => {
            return this.calificacionService.listar();
          })
        ).subscribe((data) => {
          this.calificacionService.calificacionCambio.next(data);
          this.calificacionService.mensajeCambio.next("una calificación fue modificada!");
        });
    } else {
      this.calificacionService.registrar(this.calificacion).pipe(
          switchMap(() => {
            return this.calificacionService.listar();
          })
        ).subscribe((data) => {
          this.calificacionService.calificacionCambio.next(data);
          this.calificacionService.mensajeCambio.next("Se registró una calificación!");
        });
    }
